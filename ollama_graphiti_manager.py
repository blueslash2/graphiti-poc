...
class OllamaGraphitiManager:
 ...
 def setup_ollama_config(self):
 """配置Ollama LLM客户端"""
        self.logger.info("配置Ollama LLM客户端...")
 # 从配置文件获取Ollama配置
        ollama_config = config.get_ollama_config()
 # 配置Ollama LLM客户端
        llm_config = LLMConfig(
            api_key=ollama_config['api_key'],  # Ollama不需要真实的API密钥
            model=ollama_config['model'],
            small_model=ollama_config['small_model'],
            base_url=f"{ollama_config['base_url']}/v1",  # Ollama提供的端口
        )
        self.llm_client = OpenAIClient(config=llm_config)
        self.logger.info("Ollama LLM客户端配置完成")
 return llm_config
 ...
 # 从配置文件获取Ollama配置
        ...
        self.embedder = OpenAIEmbedder(
            config=OpenAIEmbedderConfig(
                ...
                embedding_dim=ollama_config['embedding_dim'],  # bge-large-zh-v1.5是1024维
